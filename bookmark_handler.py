...
def lambda_handler(event, context):
    ...
    # HTTP API(v2)와 REST API 방식 모두 대응
        ...
        # 2. 찜하기 토글 로직 (POST)
            ...
            if 'Item' in response:
                ...
                return {
                    'statusCode': 200,
                    'headers': headers,
                    'body': json.dumps({'message': '찜 취소됨', 'status': 'removed', 'userId': user_id}, ensure_ascii=False)
                }
            else:
                # 없다면 등록 (찜 하기)
                table.put_item(Item={
                ...
                return {
                    'statusCode': 200,
                    'headers': headers,
                    'body': json.dumps({'message': '찜 완료', 'status': 'added', 'userId': user_id}, ensure_ascii=False)
                }
